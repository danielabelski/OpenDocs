...
def _default_branch(repo_dir: Path) -> str | None:
    ...
    Returns *None* when it cannot be determined, in which case callers should
    let the tool they are invoking apply its own default.
    """
    result = _git_run(repo_dir, "symbolic-ref", "refs/remotes/origin/HEAD")
    ...
    result = _git_run(repo_dir, "remote", "show", "origin")
    if result.returncode == 0:
...
def _restore(repo_dir: Path, stashed: bool, *, checkout_back: bool = True) -> None:
    """Return the repo to its pre-run state.

    Always call this before leaving :func:`_create_docs_pr`, on success and on
    every failure path alike — otherwise a failed push would silently leave the
    user's uncommitted work sitting in the stash.
    """
    if checkout_back:
        _git_run(repo_dir, "checkout", "-")
    if stashed:
        result = _git_run(repo_dir, "stash", "pop")
        if result.returncode != 0:
            log.error(
                "Could not restore stashed changes (%s). Your work is safe in "
                "`git stash list` — run `git stash pop` to recover it.",
                result.stderr.strip(),
            )
...
def _create_docs_pr(
    ...
    stashed = False
    try:
        # Ensure we're on a clean state.  git reports "No local changes to save"
        # (exit 0) when there was nothing to stash — only pop later if we
        # actually created a stash entry, otherwise we would pop someone else's.
        stash_result = _git_run(repo_dir, "stash", "--include-untracked")
        ...
        if result.returncode != 0:
            ...
            _restore(repo_dir, stashed, checkout_back=False)
            return False
        ...
        try:
            ...
            # Target the repo's real default branch. Hardcoding "main" breaks
            # every repo that still uses "master" (or anything else); when it
            # cannot be resolved, let `gh` pick the default itself.
        ...
        try:
            ...
            log.exception("Failed to restore working tree; run `git stash pop` manually")
        return False
